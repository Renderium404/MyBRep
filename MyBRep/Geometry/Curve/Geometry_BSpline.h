#ifndef MYBREP_GEOMETRY_CURVE_GEOMETRY_BSPLINE_H
#define MYBREP_GEOMETRY_CURVE_GEOMETRY_BSPLINE_H
...
// 表示由次数、控制点和非递减节点向量定义的完整非周期B样条曲线。
class Geometry_BSpline : public Geometry_Curve
{
public:
    // 使用指定次数、控制点和节点向量创建非周期B样条曲线。
    ...
    // 返回0，当前Geometry_BSpline只表示非周期B样条曲线。
    double period() const override;

    /// 参数查询

    // 使用de Boor算法返回指定参数对应的B样条曲线点。
...
private:
    ...
    static std::size_t findSpan(const std::vector<double>& knots,
                                ...
                                double parameter);
    // 使用de Boor算法对指定B样条数据求值。
    static MyMath::Vector3 evaluate(const std::vector<MyMath::Vector3>& controlPoints,
                                    const std::vector<double>& knots,
                                    std::size_t degree,
                                    double parameter);
    // 构造一阶导数B样条的控制点和节点向量。
    static void buildDerivativeCurve(const std::vector<MyMath::Vector3>& controlPoints,
                                     const std::vector<double>& knots,
                                     std::size_t degree,
                                     std::vector<MyMath::Vector3>& derivativeControlPoints,
                                     std::vector<double>& derivativeKnots);
    // 返回指定内部参数节点的精确重复次数，非内部节点参数返回0。
...
private:
    std::size_t m_degree; // B样条曲线次数。
    std::vector<MyMath::Vector3> m_controlPoints; // 按样条控制顺序保存的控制点。
    std::vector<double> m_knots; // 非递减节点向量。
};

}

#endif // MYBREP_GEOMETRY_CURVE_GEOMETRY_BSPLINE_H