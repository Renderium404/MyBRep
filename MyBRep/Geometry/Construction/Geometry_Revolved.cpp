...
// 返回两个二维母线连接点使用的尺度自适应有效容差。
double effectivePointTolerance(const MyMath::Vector2& first, const MyMath::Vector2& second, double tolerance)
{
    double scale = 1.0;
...
// 返回二维叉积标量。
double cross2D(const MyMath::Vector2& first, const MyMath::Vector2& second)
{
...
// 判断Circle有限参数区间是否包含指定二维母线点。
bool circleSegmentContainsPoint(const MyBRep::Geometry_Circle2D& circle, double firstParameter, double lastParameter,
                                const MyMath::Vector2& point, double tolerance)
{
    const MyMath::Vector2 relative = point - circle.center();
    const double radialDistance = stableLength(relative.x(), relative.y());

    if (std::fabs(radialDistance - circle.radius()) > tolerance) return false;
...
// 返回二维点到有限直线段的最短距离平方。
double pointSegmentDistanceSquared(const MyMath::Vector2& point, const MyMath::Vector2& start, const MyMath::Vector2& end)
{
    ...
    const MyMath::Vector2 relative = point - start;
    double parameter = (relative.x() * segment.x() + relative.y() * segment.y()) / lengthSquared;
    parameter = (std::max)(0.0, (std::min)(1.0, parameter));

    const MyMath::Vector2 closest = start + segment * parameter;
    const double deltaX = point.x() - closest.x();
    const double deltaY = point.y() - closest.y();
    return deltaX * deltaX + deltaY * deltaY;
}

// 返回二维点到Circle有限参数区间的最短距离平方。
double pointCircleSegmentDistanceSquared(const MyMath::Vector2& point, const MyBRep::Geometry_Circle2D& circle,
                                         double firstParameter, double lastParameter)
{
    const MyMath::Vector2 relative = point - circle.center();
    const double centerDistance = stableLength(relative.x(), relative.y());
    ...
    const MyMath::Vector2 start = circle.pointAt(firstParameter);
    const MyMath::Vector2 end = circle.pointAt(lastParameter);
...
// 返回有限二维直线段与向右水平射线的交点数量，queryY已数值避开轮廓顶点。
unsigned int lineHorizontalRayIntersections(const MyMath::Vector2& point, double queryY,
                                            const MyMath::Vector2& start, const MyMath::Vector2& end, double tolerance)
{
    if ((start.y() > queryY) == (end.y() > queryY)) return 0;
...
// 求Circle某个二维坐标分量等于target时的最多两个标准周期参数。
int circleCoordinateParameters(const MyBRep::Geometry_Circle2D& circle, bool solveX, double target, double parameters[2])
{
    const double cosineCoefficient = circle.radius() * (solveX ? circle.xDir().x() : circle.xDir().y());
    const double sineCoefficient = circle.radius() * (solveX ? circle.yDir().x() : circle.yDir().y());
    const double value = target - (solveX ? circle.center().x() : circle.center().y());
    const double amplitude = stableLength(cosineCoefficient, sineCoefficient);

    if (amplitude == 0.0 || value < -amplitude || value > amplitude) return 0;
    ...
    if (delta <= (std::numeric_limits<double>::epsilon)() * NumericalScale ||
        std::fabs(delta - Pi) <= (std::numeric_limits<double>::epsilon)() * NumericalScale)
    {
        return 1;
    }
...
unsigned int circleHorizontalRayIntersections(const MyMath::Vector2& point, double queryY,
                                              const MyBRep::Geometry_Circle2D& circle,
                                              double firstParameter, double lastParameter, double tolerance)
{
    double parameters[2];
    ...
    {
        if (!periodicParameterInInterval(parameters[index], TwoPi, firstParameter, lastParameter)) continue;

        const MyMath::Vector2 derivative = circle.firstDerivativeAt(parameters[index]);
        ...
        if (std::fabs(derivative.y()) <= tangentTolerance) continue;

        const MyMath::Vector2 candidate = circle.pointAt(parameters[index]);
        if (candidate.x() > point.x() + tolerance) ++intersectionCount;
    }
...
// 判断二维点是否位于母线矩形内部或边界上。
bool rectangleContainsPoint(const MyBRep::Bounds3& bounds, const MyMath::Vector2& point, double tolerance)
{
...
// 使用二维Slab算法判断有限直线段是否与母线矩形相交或接触。
bool lineSegmentIntersectsRectangle(const MyMath::Vector2& start, const MyMath::Vector2& end,
                                    const MyBRep::Bounds3& bounds, double tolerance)
{
...
// 判断Circle有限参数区间是否与母线矩形相交或接触。
bool circleSegmentIntersectsRectangle(const MyBRep::Geometry_Circle2D& circle, double firstParameter, double lastParameter,
                                      const MyBRep::Bounds3& bounds, double tolerance)
{
    const MyMath::Vector2 start = circle.pointAt(firstParameter);
    const MyMath::Vector2 end = circle.pointAt(lastParameter);

    if (rectangleContainsPoint(bounds, start, tolerance) || rectangleContainsPoint(bounds, end, tolerance)) return true;
    ...
    for (int edgeIndex = 0; edgeIndex < 2; ++edgeIndex)
    {
        ...
        for (int index = 0; index < count; ++index)
        {
            ...
            const MyMath::Vector2 candidate = circle.pointAt(parameters[index]);
            if (candidate.y() >= bounds.minimum().y() - tolerance && candidate.y() <= bounds.maximum().y() + tolerance) return true;
        }
    }
    ...
    for (int edgeIndex = 0; edgeIndex < 2; ++edgeIndex)
    {
        ...
        for (int index = 0; index < count; ++index)
        {
            ...
            const MyMath::Vector2 candidate = circle.pointAt(parameters[index]);
            if (candidate.x() >= bounds.minimum().x() - tolerance && candidate.x() <= bounds.maximum().x() + tolerance) return true;
        }
    }
...
bool Geometry_Revolved::containsLocalPoint(const MyMath::Vector3& point) const
{
    ...
    return containsProfilePoint(MyMath::Vector2(m_radialSign * radius, point.z()), m_profileTolerance);
}
...
double Geometry_Revolved::signedDistanceLocalPoint(const MyMath::Vector3& point) const
{
    ...
    const double radius = stableLength(point.x(), point.y());
    const MyMath::Vector2 profilePoint(m_radialSign * radius, point.z());
...
void Geometry_Revolved::validateProfile() const
{
    for (std::size_t index = 0; index < m_profileSegments.size(); ++index)
    {
        ...
        MYBREP_ASSERT_MESSAGE(isFiniteValue(segment.firstParameter) && isFiniteValue(segment.lastParameter),
                              "Geometry_Revolved profile segment parameters must be finite.");
        MYBREP_ASSERT_MESSAGE(segment.firstParameter != segment.lastParameter,
                              "Geometry_Revolved profile segment parameter interval must be non-degenerate.");
        MYBREP_ASSERT_MESSAGE(segment.curve->isParameterInDomain(segment.firstParameter) &&
                              segment.curve->isParameterInDomain(segment.lastParameter),
        ...
        if (segment.curve->kind() == CurveKind::Circle)
        {
            ...
            MYBREP_ASSERT_MESSAGE(sweep <= TwoPi + MyMath::Vector2::DefaultEpsilon,
                                  "Geometry_Revolved profile circle interval must not exceed one complete period.");
        }

        const ProfileSegment& next = m_profileSegments[(index + 1) % m_profileSegments.size()];
        const MyMath::Vector2 end = segmentEndPoint(segment);
        const MyMath::Vector2 nextStart = segmentStartPoint(next);
        ...
        MYBREP_ASSERT_MESSAGE(end.isEqualTo(nextStart, connectionTolerance),
                              "Geometry_Revolved profile segments must form an ordered closed loop.");
    }
}
...
void Geometry_Revolved::rebuildCaches()
{
    ...
    {
        const ProfileSegment& segment = m_profileSegments[index];
        const MyMath::Vector2 start = segmentStartPoint(segment);
        const MyMath::Vector2 end = segmentEndPoint(segment);
        ...
        const Geometry_Circle2D& circle = static_cast<const Geometry_Circle2D&>(*segment.curve);
        const MyMath::Vector2 center = circle.center();
        const double sweep = segment.lastParameter - segment.firstParameter;

        m_profileSignedArea += 0.5 * (cross2D(center, end - start) +
                                      circle.radius() * circle.radius() * circle.orientationSign() * sweep);

        const MyMath::Vector2 extrema[4] =
        {
            MyMath::Vector2(center.x() + circle.radius(), center.y()),
            MyMath::Vector2(center.x() - circle.radius(), center.y()),
            MyMath::Vector2(center.x(), center.y() + circle.radius()),
            MyMath::Vector2(center.x(), center.y() - circle.radius())
        };

        for (int extremaIndex = 0; extremaIndex < 4; ++extremaIndex)
        {
            if (!circleSegmentContainsPoint(circle, segment.firstParameter, segment.lastParameter, extrema[extremaIndex], m_profileTolerance)) continue;
            m_profileBounds.include(MyMath::Vector3(extrema[extremaIndex].x(), extrema[extremaIndex].y(), 0.0));
        }
    }

    MYBREP_ASSERT_MESSAGE(m_profileBounds.isValid(), "Geometry_Revolved profile bounds must be valid.");
    MYBREP_ASSERT_MESSAGE(isFiniteValue(m_profileSignedArea) && m_profileSignedArea != 0.0,
                          "Geometry_Revolved profile must enclose a non-zero signed area.");
    ...
    MYBREP_ASSERT_MESSAGE(onPositiveSide || onNegativeSide,
                          "Geometry_Revolved profile must remain entirely on one side of the local Y axis.");
    ...
    MYBREP_ASSERT_MESSAGE(bounds.isValid() && bounds.hasVolume(),
                          "Geometry_Revolved profile must define a finite non-degenerate three-dimensional solid.");

    setLocalBounds(bounds);
}

MyMath::Vector2 Geometry_Revolved::segmentStartPoint(const ProfileSegment& segment) const
{
    return segment.curve->pointAt(segment.firstParameter);
}

MyMath::Vector2 Geometry_Revolved::segmentEndPoint(const ProfileSegment& segment) const
{
...
bool Geometry_Revolved::containsProfilePoint(const MyMath::Vector2& point, double tolerance) const
{
    MYBREP_ASSERT_MESSAGE(point.isFinite(), "Geometry_Revolved profile query point must be finite.");
    ...
    if (point.x() < m_profileBounds.minimum().x() - tolerance || point.x() > m_profileBounds.maximum().x() + tolerance ||
        point.y() < m_profileBounds.minimum().y() - tolerance || point.y() > m_profileBounds.maximum().y() + tolerance)
    {
        return false;
    }
    ...
    for (std::size_t index = 0; index < m_profileSegments.size(); ++index)
    {
        ...
        if (segment.curve->kind() == CurveKind::Line)
        {
            if (pointSegmentDistanceSquared(point, segmentStartPoint(segment), segmentEndPoint(segment)) <= toleranceSquared) return true;
            continue;
        }

        const Geometry_Circle2D& circle = static_cast<const Geometry_Circle2D&>(*segment.curve);

        if (pointCircleSegmentDistanceSquared(point, circle, segment.firstParameter, segment.lastParameter) <= toleranceSquared) return true;
    }
...
double Geometry_Revolved::profileBoundaryDistance(const MyMath::Vector2& point) const
{
    MYBREP_ASSERT_MESSAGE(point.isFinite(), "Geometry_Revolved profile distance query point must be finite.");
    ...
    MYBREP_ASSERT_MESSAGE(isFiniteValue(minimumDistanceSquared) && minimumDistanceSquared >= 0.0,
                          "Geometry_Revolved failed to resolve a finite profile boundary distance.");
...
{
    if (!m_profileBounds.intersects(bounds, m_profileTolerance)) return ShapeRelation::Outside;

    const MyMath::Vector2 corners[4] =
    {
        MyMath::Vector2(bounds.minimum().x(), bounds.minimum().y()),
        MyMath::Vector2(bounds.maximum().x(), bounds.minimum().y()),
        MyMath::Vector2(bounds.maximum().x(), bounds.maximum().y()),
        MyMath::Vector2(bounds.minimum().x(), bounds.maximum().y())
    };
    ...
    if (boundaryIntersects) return ShapeRelation::Intersecting;
    if (allCornersInside) return ShapeRelation::Inside;
    if (anyCornerInside) return ShapeRelation::Intersecting;
...
ShapeRelation Geometry_Revolved::classifyRange(const MyMath::Vector3& minimum, const MyMath::Vector3& maximum) const
{
    ...
    const Bounds3 profileQueryBounds(MyMath::Vector3(profileMinimumX, minimum.z(), 0.0),
                                     MyMath::Vector3(profileMaximumX, maximum.z(), 0.0));

    const ShapeRelation relation = classifyProfileBounds(profileQueryBounds);

    if (relation == ShapeRelation::Outside) return ShapeRelation::Outside;
    if (relation == ShapeRelation::Inside) return ShapeRelation::Inside;
