...
bool sampleEdge(const MyBRep::Topology_Edge& edge, const MyBRep::Geometry_Surface& surface, const MyBRep::SphericalFaceMeshOptions& options, std::vector<MyMath::Vector2>& points)
{
    points.clear();
...
bool parametersSharePole(const MyBRep::Geometry_SphericalSurface& sphere, const MyMath::Vector2& first, const MyMath::Vector2& second, double tolerance)
{
...
bool alignEdgeSampleToPrevious(std::vector<MyMath::Vector2>& edgePoints, const MyMath::Vector2& previousPoint,
                               const MyBRep::Geometry_SphericalSurface& sphere, double tolerance)
{
    if (edgePoints.empty())
    {
        return false;
    ...
    if (parametersSharePole(sphere, previousPoint, edgePoints.front(), tolerance))
    {
        return true;
    }

    const double period = sphere.uPeriod();
...
bool sampleWire(const MyBRep::Topology_Wire& wire, const MyBRep::Geometry_SphericalSurface& sphere, const MyBRep::SphericalFaceMeshOptions& options,
                std::vector<MyMath::Vector2>& points)
{
    points.clear();
    ...
    for (std::size_t edgeIndex = 0; edgeIndex < wire.edgeCount(); ++edgeIndex)
    {
        ...
        if (!sampleEdge(wire.edge(edgeIndex), sphere, options, edgePoints))
        {
            return false;
        }
        ...
        if (!points.empty())
        {
            if (!alignEdgeSampleToPrevious(edgePoints, points.back(), sphere, options.geometricTolerance))
            {
                return false;
            }
    ...
    const bool closesAtPole = parametersSharePole(sphere, points.front(), points.back(), options.geometricTolerance);

    if (!closesInParameterSpace && !closesAtPole)
    {
        return false;
    }
...
double surfaceEdgeChordError(const MyBRep::Geometry_SphericalSurface& sphere, const std::vector<MyMath::Vector2>& vertices, const EdgeKey& edge)
{
    const MyMath::Vector2 firstParameter = vertices[edge.first];
    const MyMath::Vector2 secondParameter = vertices[edge.second];

    if (parametersSharePole(sphere, firstParameter, secondParameter, MyMath::Vector2::DefaultEpsilon))
    {
        return 0.0;
    }

    const MyMath::Vector2 middleParameter = (firstParameter + secondParameter) * 0.5;
    const MyMath::Vector3 first = sphere.pointAt(firstParameter.x(), firstParameter.y());
    const MyMath::Vector3 second = sphere.pointAt(secondParameter.x(), secondParameter.y());
    const MyMath::Vector3 middle = sphere.pointAt(middleParameter.x(), middleParameter.y());

    return pointSegmentDistance3D(middle, first, second);
}

bool edgeNeedsSurfaceSplit(const MyBRep::Geometry_SphericalSurface& sphere, const std::vector<MyMath::Vector2>& vertices, const EdgeKey& edge, double chordTolerance)
{
    return surfaceEdgeChordError(sphere, vertices, edge) > chordTolerance;
}
...
bool refineSurfaceOnce(const MyBRep::Geometry_SphericalSurface& sphere, std::vector<MyMath::Vector2>& vertices, std::vector<IndexedTriangle>& triangles, double chordTolerance, bool& changed)
{
    changed = false;
    ...
    for (std::size_t index = 0; index < triangles.size(); ++index)
    {
        ...
        if (edgeNeedsSurfaceSplit(sphere, vertices, firstSecond, chordTolerance))
        {
            splitEdges.insert(firstSecond);
        }

        if (edgeNeedsSurfaceSplit(sphere, vertices, secondThird, chordTolerance))
        {
            splitEdges.insert(secondThird);
        }

        if (edgeNeedsSurfaceSplit(sphere, vertices, thirdFirst, chordTolerance))
        {
            splitEdges.insert(thirdFirst);
        }
    }
...
bool surfaceNeedsMoreRefinement(const MyBRep::Geometry_SphericalSurface& sphere, const std::vector<MyMath::Vector2>& vertices, const std::vector<IndexedTriangle>& triangles, double chordTolerance)
{
    for (std::size_t index = 0; index < triangles.size(); ++index)
    {
        const IndexedTriangle& triangle = triangles[index];

        if (edgeNeedsSurfaceSplit(sphere, vertices, EdgeKey(triangle.first, triangle.second), chordTolerance) ||
            edgeNeedsSurfaceSplit(sphere, vertices, EdgeKey(triangle.second, triangle.third), chordTolerance) ||
            edgeNeedsSurfaceSplit(sphere, vertices, EdgeKey(triangle.third, triangle.first), chordTolerance))
        {
            return true;
        }
    }
...
bool refineSphericalSurface(const MyBRep::Geometry_SphericalSurface& sphere, const MyBRep::SphericalFaceMeshOptions& options,
                            std::vector<MyMath::Vector2>& vertices, std::vector<IndexedTriangle>& triangles)
{
    for (int round = 0; round < options.maximumSurfaceSubdivisionRounds; ++round)
    {
        ...
        if (!refineSurfaceOnce(sphere, vertices, triangles, options.surfaceChordTolerance, changed))
        {
            return false;
        }
    ...
    return !surfaceNeedsMoreRefinement(sphere, vertices, triangles, options.surfaceChordTolerance);
}

bool trianglePositionsAreNonDegenerate(const MyMath::Vector3& first, const MyMath::Vector3& second, const MyMath::Vector3& third)
{
...
MyBRep::FaceMesh buildFaceMesh(const MyBRep::Topology_Face& face, const MyBRep::Geometry_SphericalSurface& sphere,
                               const std::vector<MyMath::Vector2>& parameterVertices, const std::vector<IndexedTriangle>& triangles, double tolerance)
{
    ...
    std::vector<unsigned int> outputIndices(parameterVertices.size(), 0);
    ...
    {
        const MyMath::Vector2 parameter = canonicalPoleParameter(sphere, parameterVertices[index], tolerance);
        ...
        const unsigned int outputIndex = result.addVertex(MyBRep::FaceMeshVertex(parameter, position, normal));
    ...
    {
        const IndexedTriangle& triangle = triangles[index];
        const unsigned int first = outputIndices[triangle.first];
        const unsigned int second = outputIndices[triangle.second];
        const unsigned int third = outputIndices[triangle.third];
        ...
        const MyMath::Vector3& firstPosition = result.vertices()[first].position;
        const MyMath::Vector3& secondPosition = result.vertices()[second].position;
        const MyMath::Vector3& thirdPosition = result.vertices()[third].position;

        if (!trianglePositionsAreNonDegenerate(firstPosition, secondPosition, thirdPosition))
        {
            continue;
        }
...
SphericalFaceMeshOptions::SphericalFaceMeshOptions()
    ...
    , surfaceChordTolerance(1.0e-3)                       // 默认1e-3模型单位球面共享边弦高误差，控制两个参数方向和斜边曲率细分。
    , geometricTolerance(MyMath::Vector2::DefaultEpsilon) // UV连接、周期展开和极点识别沿用MyMath二维基础比较误差。
    ...
    , maximumSurfaceSubdivisionRounds(12)                // 最多12轮共享边一致细分，兼顾较大球面片和小弦高误差。
{
}

bool SphericalFaceMeshOptions::isValid() const
{
...
{
    if (!face.isValid() || face.geometry().kind() != SurfaceKind::Spherical || face.wireCount() == 0)
    {
        return false;
    }
    ...
    for (std::size_t wireIndex = 0; wireIndex < face.wireCount(); ++wireIndex)
    {
        ...
        for (std::size_t edgeIndex = 0; edgeIndex < wire.edgeCount(); ++edgeIndex)
        {
            ...
            if (!edge.hasCurveOnSurface(sphere))
            {
                return false;
            }
        }
...
FaceMesh SphericalFaceMesher::mesh(const Topology_Face& face, const SphericalFaceMeshOptions& options)
{
    ...
    for (std::size_t wireIndex = 0; wireIndex < face.wireCount(); ++wireIndex)
    {
        ...
        if (!sampleWire(face.wire(wireIndex), sphere, options, ring.points))
        {
            return FaceMesh();
        }
    ...
    if (!refineSphericalSurface(sphere, options, parameterVertices, indexedTriangles))
    {
        return FaceMesh();
    }

    return buildFaceMesh(face, sphere, parameterVertices, indexedTriangles, options.geometricTolerance);
}
