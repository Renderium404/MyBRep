...
void removeSimpleCollinearPoints(std::vector<MyMath::Vector2>& points, double tolerance)
{
 ...
 if (absoluteValue(orientation(points[previousIndex], points[index], points[nextIndex])) <= tolerance && pointOnSegment(points[index], points[previousIndex], points[nextIndex], tolerance))
            {
...
bool sampleEdge(const MyBRep::Topology_Edge& edge, const MyBRep::Geometry_Surface& surface, const MyBRep::ConicalFaceMeshOptions& options, std::vector<MyMath::Vector2>& points)
{
    points.clear();
...
bool parameterTouchesApex(const MyBRep::Geometry_ConicalSurface& cone, const MyMath::Vector2& parameter, double tolerance)
{
    return parameter.y() <= cone.vDomainStart() + tolerance;
}

bool parametersShareApex(const MyBRep::Geometry_ConicalSurface& cone, const MyMath::Vector2& first, const MyMath::Vector2& second, double tolerance)
{
    return parameterTouchesApex(cone, first, tolerance) && parameterTouchesApex(cone, second, tolerance);
}

MyMath::Vector3 coneLimitNormal(const MyBRep::Topology_Face& face, const MyBRep::Geometry_ConicalSurface& cone, double u)
{
...
bool alignEdgeSampleToPrevious(std::vector<MyMath::Vector2>& edgePoints, const MyMath::Vector2& previousPoint, const MyBRep::Geometry_ConicalSurface& cone, double tolerance)
{
    if (edgePoints.empty())
    {
        return false;
    ...
    if (parametersShareApex(cone, previousPoint, edgePoints.front(), tolerance))
    {
        return true;
    }

    const double period = cone.uPeriod();
    const double shiftCount = nearestInteger((previousPoint.x() - edgePoints.front().x()) / period);
    shiftPointsU(edgePoints, shiftCount * period);

    return pointsEqual(previousPoint, edgePoints.front(), tolerance);
}

bool sampleWire(const MyBRep::Topology_Wire& wire, const MyBRep::Geometry_ConicalSurface& cone, const MyBRep::ConicalFaceMeshOptions& options, std::vector<MyMath::Vector2>& points)
{
    points.clear();
    ...
    for (std::size_t edgeIndex = 0; edgeIndex < wire.edgeCount(); ++edgeIndex)
    {
        ...
        if (!sampleEdge(wire.edge(edgeIndex), cone, options, edgePoints))
        {
            return false;
        }
        ...
        if (!points.empty())
        {
            if (!alignEdgeSampleToPrevious(edgePoints, points.back(), cone, options.geometricTolerance))
            {
                return false;
            }
    ...
    const bool closesInParameterSpace = pointsEqual(points.front(), points.back(), options.geometricTolerance);
    const bool closesAtApex = parametersShareApex(cone, points.front(), points.back(), options.geometricTolerance);

    if (!closesInParameterSpace && !closesAtApex)
    {
        return false;
    }
...
bool removeOneDegenerateVertex(std::vector<MyMath::Vector2>& polygon, double tolerance)
{
 ...
 if (absoluteValue(orientation(polygon[previousIndex], polygon[index], polygon[nextIndex])) <= tolerance && pointOnSegment(polygon[index], polygon[previousIndex], polygon[nextIndex], tolerance))
        {
            polygon.erase(polygon.begin() + index);
            return true;
        }
...
double surfaceEdgeChordError(const MyBRep::Geometry_ConicalSurface& cone, const std::vector<MyMath::Vector2>& vertices, const EdgeKey& edge)
{
    const MyMath::Vector2 firstParameter = vertices[edge.first];
    const MyMath::Vector2 secondParameter = vertices[edge.second];

    if (parametersShareApex(cone, firstParameter, secondParameter, MyMath::Vector2::DefaultEpsilon))
    {
        return 0.0;
    }

    const MyMath::Vector2 middleParameter = (firstParameter + secondParameter) * 0.5;
    const MyMath::Vector3 first = cone.pointAt(firstParameter.x(), firstParameter.y());
    const MyMath::Vector3 second = cone.pointAt(secondParameter.x(), secondParameter.y());
    const MyMath::Vector3 middle = cone.pointAt(middleParameter.x(), middleParameter.y());

    return pointSegmentDistance3D(middle, first, second);
}

bool edgeNeedsSurfaceSplit(const MyBRep::Geometry_ConicalSurface& cone, const std::vector<MyMath::Vector2>& vertices, const EdgeKey& edge, double chordTolerance)
{
    return surfaceEdgeChordError(cone, vertices, edge) > chordTolerance;
}
...
bool refineSurfaceOnce(const MyBRep::Geometry_ConicalSurface& cone, std::vector<MyMath::Vector2>& vertices, std::vector<IndexedTriangle>& triangles, double chordTolerance, bool& changed)
{
    changed = false;
    ...
    for (std::size_t index = 0; index < triangles.size(); ++index)
    {
        ...
        if (edgeNeedsSurfaceSplit(cone, vertices, firstSecond, chordTolerance))
        {
            splitEdges.insert(firstSecond);
        }

        if (edgeNeedsSurfaceSplit(cone, vertices, secondThird, chordTolerance))
        {
            splitEdges.insert(secondThird);
        }

        if (edgeNeedsSurfaceSplit(cone, vertices, thirdFirst, chordTolerance))
        {
            splitEdges.insert(thirdFirst);
        }
    }
...
bool surfaceNeedsMoreRefinement(const MyBRep::Geometry_ConicalSurface& cone, const std::vector<MyMath::Vector2>& vertices, const std::vector<IndexedTriangle>& triangles, double chordTolerance)
{
    for (std::size_t index = 0; index < triangles.size(); ++index)
    {
        const IndexedTriangle& triangle = triangles[index];

        if (edgeNeedsSurfaceSplit(cone, vertices, EdgeKey(triangle.first, triangle.second), chordTolerance) ||
            edgeNeedsSurfaceSplit(cone, vertices, EdgeKey(triangle.second, triangle.third), chordTolerance) ||
            edgeNeedsSurfaceSplit(cone, vertices, EdgeKey(triangle.third, triangle.first), chordTolerance))
        {
            return true;
        }
    }
...
bool refineConicalSurface(const MyBRep::Geometry_ConicalSurface& cone, const MyBRep::ConicalFaceMeshOptions& options,
                            std::vector<MyMath::Vector2>& vertices, std::vector<IndexedTriangle>& triangles)
{
    for (int round = 0; round < options.maximumSurfaceSubdivisionRounds; ++round)
    {
        ...
        if (!refineSurfaceOnce(cone, vertices, triangles, options.surfaceChordTolerance, changed))
        {
            return false;
        }
    ...
    return !surfaceNeedsMoreRefinement(cone, vertices, triangles, options.surfaceChordTolerance);
}
...
MyBRep::FaceMesh buildFaceMesh(const MyBRep::Topology_Face& face, const MyBRep::Geometry_ConicalSurface& cone,
                                const std::vector<MyMath::Vector2>& parameterVertices, const std::vector<IndexedTriangle>& triangles, double tolerance)
{
    MyBRep::FaceMesh result;

    for (std::size_t index = 0; index < parameterVertices.size(); ++index)
    {
        MyMath::Vector2 parameter = parameterVertices[index];

        if (parameterTouchesApex(cone, parameter, tolerance))
        {
            parameter.setY(cone.vDomainStart());
        }

        const MyMath::Vector3 position = cone.pointAt(parameter.x(), parameter.y());
        const MyMath::Vector3 normal = parameterTouchesApex(cone, parameter, tolerance) ? coneLimitNormal(face, cone, parameter.x()) : face.normalAt(parameter.x(), parameter.y());
    ...
    for (std::size_t index = 0; index < triangles.size(); ++index)
    {
        ...
        if (!triangleIsNonDegenerate3D(cone, parameterVertices[triangle.first], parameterVertices[triangle.second], parameterVertices[triangle.third]))
        {
            continue;
        }
...
ConicalFaceMeshOptions::ConicalFaceMeshOptions()
    ...
    , surfaceChordTolerance(1.0e-3)                       // 默认1e-3模型单位圆锥面共享边弦高误差，控制两个参数方向和斜边曲率细分。
    , geometricTolerance(MyMath::Vector2::DefaultEpsilon) // UV连接、周期展开和圆锥顶点检查沿用MyMath二维基础比较误差。
    ...
    , maximumSurfaceSubdivisionRounds(12)                // 最多12轮共享边一致细分，兼顾较大圆锥面片和小弦高误差。
{
}

bool ConicalFaceMeshOptions::isValid() const
{
    return isFiniteValue(boundaryChordTolerance) && boundaryChordTolerance > 0.0 &&
           ...
           minimumBoundarySubdivisionDepth >= 0 && maximumBoundarySubdivisionDepth >= minimumBoundarySubdivisionDepth &&
           maximumBoundarySubdivisionDepth <= MaximumAllowedBoundarySubdivisionDepth &&
           maximumSurfaceSubdivisionRounds > 0 && maximumSurfaceSubdivisionRounds <= MaximumAllowedSurfaceSubdivisionRounds;
}

bool ConicalFaceMesher::canMesh(const Topology_Face& face)
{
    if (!face.isValid() || face.geometry().kind() != SurfaceKind::Conical || face.wireCount() == 0)
    {
        return false;
    }

    const Geometry_ConicalSurface& cone = static_cast<const Geometry_ConicalSurface&>(face.geometry());

    if (!cone.isUPeriodic() || cone.uPeriod() <= 0.0 || cone.isVPeriodic())
    {
        return false;
    }
    ...
    for (std::size_t wireIndex = 0; wireIndex < face.wireCount(); ++wireIndex)
    {
        ...
        for (std::size_t edgeIndex = 0; edgeIndex < wire.edgeCount(); ++edgeIndex)
        {
            ...
            if (!edge.hasCurveOnSurface(cone))
            {
                return false;
            }
        }
...
FaceMesh ConicalFaceMesher::mesh( const Topology_Face& face, const ConicalFaceMeshOptions& options)
{
    ...
    for (std::size_t wireIndex = 0; wireIndex < face.wireCount(); ++wireIndex)
    {
        ...
        if (!sampleWire(face.wire(wireIndex), cone, options, ring.points))
        {
            return FaceMesh();
        }
    ...
    if (!refineConicalSurface(cone, options, parameterVertices, indexedTriangles))
    {
        return FaceMesh();
    }

    return buildFaceMesh(face, cone, parameterVertices, indexedTriangles, options.geometricTolerance);
}
