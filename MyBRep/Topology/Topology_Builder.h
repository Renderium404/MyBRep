...
// 提供共享拓扑实体构造阶段的受控写入口，公开Topology_Object句柄本身保持只读语义。
class Topology_Builder
{
public:
    /// Edge曲面表示

    // 为共享Edge增加指定Surface上的单条二维参数曲线表示。
    static void addCurveOnSurface(Topology_Edge& edge,
    ...
    // 为共享Edge增加闭合Surface seam上的两条二维参数曲线表示。
    static void addCurveOnClosedSurface(Topology_Edge& edge,
                                        const Foundation::RefPtr<const Geometry_Surface>& surface,
...
private:
    ...
    static void validateCurveOnSurface(const Topology_TEdge& edge,
